...
2. Check: Brandon's phone + Cid's phone charged, both on cellular; Cid signed into
   `dealer.carnimbus.com` on the laptop; Brandon logged OUT (fresh sign-in on camera).

## The 10 steps
1. **Homepage** — `carnimbus.com`. New "old way vs CarNimbus way" visuals. Tap **Log in**.
...
- **QR won't scan (lighting):** Cid types the 6-char desk code on `dealer.carnimbus.com/scan`.
