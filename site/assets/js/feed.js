document.addEventListener("DOMContentLoaded",function(){
  ...
  function load(){fetch("/api/comments").then(function(r){return r.json();}).then(function(d){
    ...
    list.innerHTML=cs.map(function(c){
      ...
      var chip=c.year?('<a href="/car?id='+c.vdp_id+'" style="display:flex;align-items:center;gap:8px;margin-top:8px;background:rgba(6,16,40,.6);border:1px solid rgba(24,200,255,.18);border-radius:10px;padding:7px;text-decoration:none">'+
