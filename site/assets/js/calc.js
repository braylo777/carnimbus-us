document.addEventListener("DOMContentLoaded",function(){
    ...
    fetch("/api/search?monthly="+mo+"&down="+dn+"&zip="+z+"&radius="+rad+"&deal_type="+deal_type).then(function(r){return r.json();}).then(function(d){
      ...
      if(geo.length){
        ...
        mapHtml='<div style="border:1px solid rgba(24,200,255,.2);border-radius:14px;background:#081a42;margin-bottom:10px;padding:8px">'+
          '<div style="font:700 9px Manrope;color:#8ca0c4;letter-spacing:.08em;margin:0 0 4px 4px">'+(es?"DONDE ESTÁN TUS AUTOS · ★ = TÚ":"WHERE YOUR CARS ARE · ★ = YOU")+'</div>'+
      ...
      $("calc-out").innerHTML=bHtml+mapHtml+'<div style="font:600 11px Manrope;color:#8ca0c4;margin:2px 0 8px">'+(d.count!=null?d.count:cars.length)+countText+((d.count||0)>cars.length?showingText+cars.length:'')+' →</div>'+
  ...
  dealButtons.forEach(function(btn){
    btn.addEventListener("click", function(){
      dealButtons.forEach(function(b){ b.classList.remove("on"); });
